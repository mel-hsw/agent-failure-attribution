...
- **Step 4 (run Google ADK eval suite)**:
  ...
  - Phase C: three evaluators implemented and run on the 123-case eval split. Judge: `gemini-3.1-pro-preview` on Vertex `location=global`.
    - C.1 AllAtOnce (`phase_c_all_at_once.py`): **complete** — cluster 0.358 / level 0.553 / step tol-3 0.667 / P3 late-symptom 0.750. Best overall.
    - C.2 BinarySearch (`phase_c_binary_search.py`): **quota-blocked** — preview-model daily quota exhausted across four online-async attempts; only 18/123 cases succeeded. On the successful subset: cluster 0.389 / level 0.500 / tol-3 0.500. Unreportable as primary; needs oracle-batch rewrite (submit every step-index query up front) or quota increase.
    - C.3 ConstraintGrounded (`phase_c_constraint_grounded.py`): **complete (post-ID-alignment fix)** — cluster 0.341 / level **0.626** / tol-3 0.650 / P3 late-symptom 0.625. Canonical run: `outputs/phase_c/constraint_grounded/eval/phase-c-cg-eval-20260419T113252-c4fd41/`. Previously-reported cluster 0.114 / level 0.472 / tol-3 0.496 were artifacts of a Vertex-batch row-order parser on scrambled output — fixed in commit 6756a92 (align by embedded trajectory_id). Plus ablation (`--no-violation-log`): cluster 0.106 / level 0.480 / tol-3 0.423. **The violation log is the source of C.3's level-accuracy win** — ablation matches Phase B's 0.480 level exactly, while the full log lifts it to 0.626 (+14.6pp).
    - Resume helper: `phase_c_resume.py` recovers runs whose local poller died mid-job (exit 144) while the Vertex batch kept running server-side.
  - Phase D: **complete** — `phase_d_scorecard.py` aggregates all runs, produces `docs/reports/step4_results.md` + JSON sidecar. Three-part match per CLAUDE.md (origin-step, cluster, P3 late-symptom), stratified by source × cluster. Calibration κ on n=5 is below 0.70 threshold for every method (limiting factor is calibration-set size, not method quality). Full design in `reports/step4_plan.md`. ADK reference notes in `reports/adk_eval_suite_notes.md`.
...
|---|---|---|
| 2026-04-19 | Phase D scorecard landed; `docs/reports/step4_results.md` written | Headline: C.1 AllAtOnce wins overall (cluster 0.358 / tol-3 0.667). C.3 ConstraintGrounded wins P3 late-symptom fidelity (0.875 vs C.1's 0.75), validating AgentRx-style evidence grounding on cascading-failure cases. C.2 is quota-blocked. Calibration κ below 0.70 — needs larger calibration pass before paper-ready claims. |
| 2026-04-19 | C.2 BinarySearchAttribution hit `gemini-3.1-pro-preview` daily quota mid-run (only 18/123 successful across 4 attempts at parallelism 1/2/4) | Preview-model quotas are stricter than GA. The online-async design can't throttle below quota floor when daily cap is hit. Path forward: rewrite as "oracle batch" — submit every step-index query for every trajectory as a single Vertex batch, reconstruct bisection path locally from verdicts. Tradeoff: O(n) calls vs O(log n), but uses batch quota path which is separate and higher. |
| 2026-04-19 | Added `scripts/phase_c_resume.py` to harvest Vertex batches whose local Python pollers were killed (exit 144 = SIGURG from harness) | Vertex batch jobs survive local-process death. Resume script polls GCS for predictions.jsonl under the original dest URI, downloads, and re-builds local artifacts. Handles both single-batch all-at-once and two-pass constraint-grounded runs (re-runs static constraints locally, builds Pass 2 input from downloaded Pass 1 predictions). |
