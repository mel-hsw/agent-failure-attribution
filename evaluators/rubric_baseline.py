"""Rubric-based failure-attribution baseline via Vertex batch.

Honors the spirit of ADK's `rubric_based_final_response_quality_v1`: nine
yes/no rubrics (one per cluster), no origin-step output.

Usage:
    python3 evaluators/rubric_baseline.py --evalset examples/minimal.with_gt.evalset.json
...
from common import resolve_evalset_path, resolve_output_dir, rubric_file  # noqa: E402
import batch_utils as bu  # noqa: E402

OUTPUT_DIR = REPO_ROOT / "outputs" / "rubric_baseline"
...
def main() -> int:
    ...
    parser.add_argument("--evalset", default=None, help="Path to *.with_gt.evalset.json")
    parser.add_argument("--split", choices=("dev", "calibration", "eval"), default=None)
    parser.add_argument("--data-dir", default=None, help="Benchmark root (default: BENCHMARK_DATA_DIR or local/benchmark)")
    ...
    if not args.evalset and not args.split:
        parser.error("Provide --evalset or --split")
    ...
    evalset_path = resolve_evalset_path(
        evalset=args.evalset, split=args.split, with_gt=True, data_dir=data_dir
    )
    ...
    run_label = args.split or evalset_path.stem.replace(".with_gt", "")
    print(f"Rubric baseline | evalset={evalset_path.name} | n={len(cases)} | model={args.judge_model}")
    ...
    run_id = bu.new_run_id(f"rubric-{run_label}")
    out_dir = resolve_output_dir(
        output=args.output,
        default_parent=OUTPUT_DIR / run_label,
        run_id=run_id,
    )
    ...
    print(f"\n=== Rubric baseline summary ({run_label}) ===")
