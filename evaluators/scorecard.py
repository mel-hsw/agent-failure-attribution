...
Invocation:
    python3 evaluators/scorecard.py \\
        --runs outputs/rubric_baseline/eval/*/per_case.jsonl \\
               outputs/all_at_once/eval/*/per_case.jsonl \\
               outputs/constraint_grounded/eval/*/per_case.jsonl \\
        --labels rubric_baseline all_at_once constraint_grounded \\
        --output outputs/scorecards/scorecard.md

The default invocation with no args auto-discovers the latest run under each
evaluator output path and writes `outputs/scorecards/scorecard.md`.
"""
...
from common import default_benchmark_data_dir, resolve_evalset_path  # noqa: E402

OUTPUT_DIR = REPO_ROOT / "outputs" / "scorecards"
...
def auto_discover(split: str) -> list[tuple[str, Path]]:
    candidates = [
        ("rubric_baseline", f"outputs/rubric_baseline/{split}/*/per_case.jsonl"),
        ("all_at_once", f"outputs/all_at_once/{split}/*/per_case.jsonl"),
        ("constraint_grounded", f"outputs/constraint_grounded/{split}/*/per_case.jsonl"),
    ]
...
def render_markdown(split: str, scores: list[dict], kappa_rows: list[dict]) -> str:
    lines = []
    lines.append(f"# Failure attribution scorecard\n")
    lines.append(f"_Generated by `evaluators/scorecard.py`. Split: `{split}`._\n")
...
def main() -> int:
    ...
    parser.add_argument("--evalset", default=None, help="Path to *.with_gt.evalset.json for ground-truth lookup")
    parser.add_argument("--data-dir", default=None)
    parser.add_argument("--split", default="eval", help="Primary split label (for auto-discover and reporting).")
    ...
    parser.add_argument(
        "--output",
        default=str(OUTPUT_DIR / "scorecard.md"),
    )
    ...
    if args.evalset:
        evalset_path = Path(args.evalset).expanduser().resolve()
    else:
        evalset_path = resolve_evalset_path(
            evalset=None, split=args.split, with_gt=True, data_dir=data_dir
        )
    ...
    cal_source_by_id = {}
    cal_evalset_path = None
    if not args.evalset:
        try:
            cal_evalset_path = resolve_evalset_path(
                evalset=None,
            ...
            cal_evalset_path = None
    if cal_evalset_path and cal_evalset_path.exists():
        cal_source_by_id = {
            c["eval_id"]: source_of(c["eval_id"])
            for c in json.loads(cal_evalset_path.read_text())["eval_cases"]
        }
