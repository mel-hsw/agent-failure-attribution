"""Constraint-grounded failure attribution via two-pass Vertex batch.

Static violation log (Python) plus LLM judge grounded in constraint evidence.

Usage:
    python3 evaluators/constraint_grounded.py --evalset examples/minimal.with_gt.evalset.json
...
OUTPUT_DIR = REPO_ROOT / "outputs" / "constraint_grounded"
...
def main() -> int:
    ...
    run_id = bu.new_run_id(f"constraint-grounded-{run_label}")
    ...
    print(
        f"ConstraintGrounded | evalset={evalset_path.name} | n={len(cases)} | "
