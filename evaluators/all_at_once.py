"""All-at-once failure attribution via Vertex batch.

One structured-JSON judgment per trajectory (origin step + cluster + level).

Usage:
    python3 evaluators/all_at_once.py --evalset examples/minimal.with_gt.evalset.json
...
from common import resolve_evalset_path, resolve_output_dir  # noqa: E402
import batch_utils as bu  # noqa: E402

OUTPUT_DIR = REPO_ROOT / "outputs" / "all_at_once"
...
def main() -> int:
    ...
    print(f"AllAtOnce | evalset={evalset_path.name} | n={len(cases)} | model={args.judge_model}")
    ...
    run_id = bu.new_run_id(f"all-at-once-{run_label}")
